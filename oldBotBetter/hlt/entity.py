...
class Ship(Entity):
    ...
    def betterNav(self, target, game_map, speed, angleStep=10):
        #do astar on the positions
        successors = self.getSuccessors(speed, angleStep)
        best = None
        bestPos = None
        for angle, pos, weight in successors:
            cost = weight + self.simpleHeuristic(pos, target) + game_map.grid[int(pos[0])][int(pos[1])]
            #speed = speed if (game_map.grid[int(pos[0])][int(pos[1])]==0) else 4
    ...
    def getSuccessors(self, speed, angleStep):
        return [(angle,
        (self.x + math.cos(math.radians(angle)) * speed, self.y + math.sin(math.radians(angle)) * speed), 
        speed) for angle in range(0,360,angleStep)]
