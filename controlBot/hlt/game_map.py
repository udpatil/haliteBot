from . import collision, entity, gVals
import logging
import sys
import math
...
class Map:
    ...
    def _link(self):
        ...
        for celestial_object in entities:
            celestial_object._link(self._players, self._planets)

        for celestial_object in entities:
            if isinstance(celestial_object, entity.Ship) and celestial_object.owner.id == self.my_id:
                #logging.info("------------------------------------------------")
                #logging.info("Celestial object: " + str(celestial_object))
                vector = (0,0)
                for obj in entities:
                    if obj != celestial_object:
                        #logging.info("my id: " + str(self.my_id))
                        #logging.info("obj is ship? id: " + str(isinstance(obj, entity.Ship)) + ", " + (str(obj.owner.id) if obj.owner != None else ""))
                        w = -1
                        if isinstance(obj, entity.Planet) and obj.owner == None:
                            g = self.gVals["Gu"]
                        elif isinstance(obj, entity.Planet) and obj.owner.id == self.my_id:
                            g = self.gVals["Gf"]
                        elif isinstance(obj, entity.Planet):
                            g = self.gVals["Ge"]
                        elif isinstance(obj, entity.Ship) and obj.owner.id == self.my_id:
                            #logging.info("friendlyShip")
                            g = self.gVals["Gfs"]
                            w = 1
                        elif isinstance(obj, entity.Ship):
                            g = self.gVals["Ges"]
                        else:
                            #logging.info("Error, none of the g conditions satisfied, obj:")
                            #logging.info(obj)
                            g = 1.0
                        expVal = -1 * g * collision.calcDistance(obj, celestial_object)
                        #logging.info(collision.calcDistance(obj, celestial_object))
                        scalar = w * math.exp(expVal)
                        #logging.info("object: " + str(obj))
                        #logging.info("scalar: " +str(scalar))
                        val = (w * g * math.exp(expVal)) / (collision.calcDistance(obj, celestial_object))
                        vector = (vector[0] - (obj.x - celestial_object.x)*val, vector[1] - (obj.y - celestial_object.y)*val)
                        #logging.info("vector: " + str(((obj.x - celestial_object.x)*val, (obj.y - celestial_object.y)*val)))
                #vector = collision.unit(vector)
                logging.info(vector)
                celestial_object.nav = vector
