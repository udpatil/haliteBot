...
class Entity:
    ...
    def __init__(self, x, y, radius, health, player, entity_id):
        ...
        self.id = entity_id
        self.coords = set()
        for i in range(int(self.x-self.radius), int(self.x+self.radius+1)):
            for j in range(int(self.y-self.radius), int(self.y+self.radius+1)):
                if collision.pointInCircle(Position(i,j), self):
                    self.coords.add((i,j))
...
class Ship(Entity):
    ...
    def betterNav(self, target, game_map, speed, angleStep=10, maxChange = 90):
        #do astar on the positions
        #calculate the angle towards the target
        direction = int(self.calculate_angle_between(target))
        successors = self.getSuccessors(speed, direction, angleStep, maxChange)
        best = None
        bestPos = None
        for angle, pos, weight in successors:
            cost = weight + self.simpleHeuristic(pos, target) + game_map.grid[int(pos[0])][int(pos[1])]
            val = game_map.obstacles_between(self, Position(pos[0],pos[1]))

            if not val and (best is None or cost < best[1]):
                best = angle, cost
                bestPos = pos
        if best is None:
            return self.thrust(0,0), (self.x,self.y)
        return self.thrust(speed, best[0]), pos

    def directionNav(self, game_map):
        #calculate the angle towards the target
        if self.nav is None:
            return self.thrust(0, 0)
        angle = math.degrees(math.atan2(self.nav[1], self.nav[0])) % 360
        obstacles = game_map.obstacles_between(self, Position(constants.MAX_SPEED*self.nav[0], constants.MAX_SPEED*self.nav[1]))
        dist = constants.MAX_SPEED
        if len(obstacles) > 0:
            for obj in obstacles:
                point = self.closest_point_to(obj, min_distance=1.5)
                distance = collision.calcDistance(self, point)
                if distance < dist:
                    dist = distance
        return self.thrust(dist, angle)



    def getSuccessors(self, speed, direction, angleStep, maxChange):

        return [(angle%360,
        (self.x + math.cos(math.radians(angle%360)) * speed, self.y + math.sin(math.radians(angle%360)) * speed), 
        speed) for angle in range(-1*maxChange + direction,maxChange+1 + direction,angleStep)]

    def simpleHeuristic(self, pos, target):
    ...
    def can_dock(self, planet):
        ...
        :rtype: bool
        """
        return (planet.owner == None or planet.owner.id == self.owner.id) and self.calculate_distance_between(planet) <= planet.radius + constants.DOCK_RADIUS + constants.SHIP_RADIUS and len(planet._docked_ship_ids) < planet.num_docking_spots
